...
namespace Injekko
{
	public sealed class InjekkoContainer : IInjekScope, IInjekBindingBuilder
	{
		...
		public void BindInstance<T>(T instance)
			=> bindings[typeof(T)] = new InstanceBinding<T>(instance);

		public void BindTransient<T>() where T : new()
			=> BindTransient<T, T>();

		public void BindTransient<TService, TImplementation>() where TImplementation : TService, new()
			=> bindings[typeof(TService)] = new TransientBinding<TService>(() => new TImplementation());

		public void BindScoped<T>() where T : new()
			=> BindScoped<T, T>();

		public void BindScoped<TService, TImplementation>() where TImplementation : TService, new()
			=> bindings[typeof(TService)] = new ScopedBinding<TService>(() => new TImplementation());
		...
		public bool TryBeginActivation(object instance)
			=> activationTracker.TryBegin(instance);

		public void CompleteActivation(object instance)
			=> activationTracker.Complete(instance);

		public void CancelActivation(object instance)
			=> activationTracker.Cancel(instance);

		public bool IsActivated(object instance)
			=> activationTracker.IsActivated(instance);
		...
		sealed class TransientBinding<T> : IInjekBinding
		{
			...
			public TransientBinding(Func<T> factory)
			{
		...
		sealed class ScopedBinding<T> : IInjekBinding
		{
			...
			public ScopedBinding(Func<T> factory)
			{
				this.factory = factory;
			}
		...
		sealed class InjekActivationTracker
		{
			...
			public bool IsActivated(object instance)
				=> instance != null && activated.Contains(instance);
		}
